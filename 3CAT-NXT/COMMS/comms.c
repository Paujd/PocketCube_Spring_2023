...
void COMMS_StateMachine( void )
{
    ...
    for(;;)                     //The only option to end the state machine is killing COMMS thread (by the OBC)
    {
        ...
        */
					...
					if (pin_correct(Buffer[0], Buffer[1]))
					{
						...
						if (Buffer[2] == TLE){
							...
							if (Buffer[2] == last_telecommand[2]){	//Second telecommand received equal to the first CHANGE THIS TO CHECK THE WHOLE TELECOMMAND. USE VARIABLE compare_arrays
								...
								Stop_timer_16();
								if ((Buffer[2] == RESET2) || (Buffer[2] == NOMINAL) || (Buffer[2] == LOW) || (Buffer[2] == CRITICAL) || (Buffer[2] == EXIT_LOW_POWER) || (Buffer[2] == EXIT_CONTINGENCY) || (Buffer[2] == EXIT_SUNSAFE) || (Buffer[2] == SET_TIME) || Buffer[2] == SEND_DATA || Buffer[2] == SEND_TELEMETRY || Buffer[2] == ACK_DATA || Buffer[2] == SEND_CALIBRATION || Buffer[2] == SEND_CONFIG || Buffer[2] == ASK_DATA|| Buffer[2] == TAKE_RF){
									telecommand_rx = false;
        ...
        //xEventGroupWaitBits(xEventGroup, COMMS_CONTINUE_EVENT, COMMS_CONTINUE_EVENT, true, portMAX_DELAY); // Blocking API function that will wait
                                                                                               // until the ADCS & PAYLOAD have done their job
        //xEventGroupSync(xEventGroup, COMMS_DONE_EVENT, 0, 0);
...
void process_telecommand(uint8_t header, uint8_t info) {
	...
	*/
	...
	case SEND_TELEMETRY:{
		...
		if (!contingency){
			Read_Flash(TELEMETRY_ADDR, &read_telemetry, sizeof(read_telemetry));
			Buffer[0] = MISSION_ID;	//Satellite ID
			Buffer[1] = POCKETQUBE_ID;	//Poquetcube ID (there are at least 3)
...
//			}
	...
	case TAKE_PHOTO:{
		...
		Send_to_WFQueue(&Buffer[3], 4, PL_TIME_ADDR, COMMSsender);
		info_write = Buffer[7];
	...
	case SEND_CONFIG:{
		...
		if (!contingency){
			...
			Send_to_WFQueue(&read_config, sizeof(read_config), CONFIG_ADDR, COMMSsender);
			Buffer[0] = MISSION_ID;	//Satellite ID
			Buffer[1] = POCKETQUBE_ID;	//Poquetcube ID (there are at least 3)
