/*
 * obc_definitions.h
 *
 *  Created on: 20 feb. 2023
 *      Author: NilRi
 */

#ifndef INC_OBC_DEFINITIONS_H_
#define INC_OBC_DEFINITIONS_H_

#define FLASH_STACK_SIZE		4300
#define EPS_STACK_SIZE		    1000
#define PAYLOAD_STACK_SIZE		1000
#define ADCS_STACK_SIZE			1000
#define COMMS_STACK_SIZE		3000
#define OBC_STACK_SIZE			2000


#define FLASH_PRIORITY			6
#define EPS_PRIORITY			5
#define PAYLOAD_PRIORITY		4
#define ADCS_PRIORITY			3
#define COMMS_PRIORITY			2
#define OBC_PRIORITY			1



#endif /* INC_OBC_DEFINITIONS_H_ */
